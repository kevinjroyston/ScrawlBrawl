    ...
    public class TextBodyBuilderGameMode : IGameMode
    {
        ...
        private static IReadOnlyDictionary<GameDuration, TimeSpan> GetGameDurationEstimates(int numPlayers, List<ConfigureLobbyRequest.GameModeOptionRequest> gameModeOptions)
        {
            ...
            foreach (GameDuration duration in Enum.GetValues(typeof(GameDuration)))
            {
                ...
                int minDrawingsRequired = TextBodyBuilderConstants.NumDrawingsInHand * 3; // the amount to make one playerHand to give everyone

                int expectedPromptsPerUser = (int)Math.Ceiling(1.0 * numPromptsPerRound * numRounds / numPlayers);
                int expectedDrawingsPerUser = Math.Max((minDrawingsRequired / numPlayers + 1) * 2, TextBodyBuilderConstants.NumDrawingsPerPlayer[duration]);
                
                int numPromptsPerUserPerRound = Math.Max(1, numPromptsPerRound / 2);

                TimeSpan estimate = TimeSpan.Zero;
                TimeSpan setupDrawingTimer = TextBodyBuilderConstants.SetupPerDrawingTimer[duration];
        ...
        public TextBodyBuilderGameMode(Lobby lobby, List<ConfigureLobbyRequest.GameModeOptionRequest> gameModeOptions, StandardGameModeOptions standardOptions)
        {
            ...
            {
                setupDrawingTimer = TextBodyBuilderConstants.SetupPerDrawingTimer[duration];
                ...
                foreach ((Prompt prompt, IGroup<User> users) in pairings)
                {
                    foreach (User user in users.Members)
                    {
                        prompt.UsersToUserHands.TryAdd(user, new Prompt.UserHand
                        {
                            ...
                            HeadChoices = MemberHelpers<PeopleUserDrawing>.Select_DynamicWeightedRandom(headDrawings, TextBodyBuilderConstants.NumDrawingsInHand),
                            BodyChoices = MemberHelpers<PeopleUserDrawing>.Select_DynamicWeightedRandom(bodyDrawings, TextBodyBuilderConstants.NumDrawingsInHand),
                            LegChoices = MemberHelpers<PeopleUserDrawing>.Select_DynamicWeightedRandom(legsDrawings, TextBodyBuilderConstants.NumDrawingsInHand),
                            Owner = user
                        });
            ...
            Func<GameState> CreateScoreGameState(List<Prompt> roundPrompts)
            {
                return () =>
                {
                    ...
                    countRounds++;
                    GameState displayPeople = new DisplayContestants_GS(
                        lobby: lobby,
        ...
        private void CountVotes(List<Person> choices, IDictionary<User, VoteInfo> votes)
        {
            ...
            {
                if (person.BodyPartDrawings[BodyPartType.Head].Owner != person.Owner) person.BodyPartDrawings[BodyPartType.Head].Owner.ScoreHolder.AddScore(TextBodyBuilderConstants.PointsForPartUsed, Score.Reason.DrawingUsed);
                if (person.BodyPartDrawings[BodyPartType.Body].Owner != person.Owner) person.BodyPartDrawings[BodyPartType.Body].Owner.ScoreHolder.AddScore(TextBodyBuilderConstants.PointsForPartUsed, Score.Reason.DrawingUsed);
                if (person.BodyPartDrawings[BodyPartType.Legs].Owner != person.Owner) person.BodyPartDrawings[BodyPartType.Legs].Owner.ScoreHolder.AddScore(TextBodyBuilderConstants.PointsForPartUsed, Score.Reason.DrawingUsed);
            }
